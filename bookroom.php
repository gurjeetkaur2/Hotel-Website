...
<body>
    ...
    if (isset($_POST['submit']) and !empty($_POST['submit']) and ($_POST['submit'] == 'Book')) {
        ...
        if ($error == 0) {
            ...
            $query = "INSERT INTO `booking` (roomID, customerID, checkin_date, checkout_date, contactNumber, booking_extras) VALUES (?,?,?,?,?,?)";

            $stmt = mysqli_prepare($DBC, $query); //prepare the query
            mysqli_stmt_bind_param($stmt, 'iissss', $room, $customer, $checkin, $checkout, $contact, $extras);
    ...
    $query = 'SELECT roomID, roomname, roomtype, bed FROM room ORDER BY roomID';
    ...
    <form class="form_settings" method="POST">
        ...
        <p>
            ...
            <input type="text" id="contact" name="contact" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}" placeholder="###-###-####" required>
        </p>
        <br>
        <p>
            <label for="extras">Booking Extras:</label>
            <textarea name="extras" id="extras" cols="35" rows="5"></textarea>
        </p>
        <br>
        <p>
    ...
    <form class="form_settings" id="searchForm" method="post" name="searching">
        <p>
